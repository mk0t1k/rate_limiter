...
- *Обработка всплесков (Burst Handling):*
  *Token Bucket* — лучший выбор. За счёт накопления токенов во время простоя он минимизирует задержку (p50) при внезапном пике нагрузки.

- *Математическая точность (Accuracy):*
  *Sliding Window Log* — идеален для критичных API. Полностью лишён проблемы «границ интервалов», гарантируя строгое соблюдение квот.
...
- *Защита от перегрузок (Overload Protection):*
  *Leaky Bucket Shaper* — лучший для сглаживания трафика. Превращает «рваный» входящий поток в стабильную и предсказуемую очередь.

- *Потребление памяти (Memory Usage):*
  *Token Bucket* требует фиксированные $O(1)$ памяти на ключ. У Sliding Window расход памяти растёт как $O(N)$, что создаёт риск OOM при Zombie-атаках.
