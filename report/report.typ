...
Разработанная библиотека реализована на стандарте C++23 и предоставляет механизмы контроля частоты запросов. Система поддерживает два фундаментальных подхода: отбрасывание превышающих лимит запросов (Rate Limiting) и их сглаживание через очереди ожидания (Traffic Shaping).

=== 2.1. Полиморфные интерфейсы и слои архитектуры

Система спроектирована с использованием строгих контрактов на базе C++20 Concepts (`RateLimiterLogic`, `Storage`, `PolyRateLimiter`) и предоставляет два независимых виртуальных интерфейса:
- *`IRateLimiter`:* Интерфейс для классических лимитеров. Предоставляет методы проверки квоты `GetNumAvail()`, метод синхронного доступа `Access(key)` и метод проверки существования ключа `Exists(key)`.
- *`IRateShaper`:* Интерфейс для алгоритмов сглаживания трафика. Метод `AddRequest(key)` возвращает объект `std::optional<std::future<bool>>`, что позволяет вызывающему коду асинхронно ожидать своей очереди на выполнение.

Архитектура разделена на базовую логику ограничений (Algorithm), систему управления ключами и примитивами синхронизации (Storage) и связующий полиморфный слой (Wrapper).

=== 2.2. Алгоритмы и механизм Traffic Shaping

Библиотека инкапсулирует логику лимитов в изолированных классах. Состояние каждого клиента обновляется в момент обращения с использованием монотонных часов `std::chrono::steady_clock`:

- *Token Bucket и Sliding Window Log:* Классические алгоритмы ограничения (Rate Limiting). Первый базируется на математической модели пополняемой емкости, второй использует очередь с приоритетом (`std::priority_queue`) для точного логгирования временных меток запросов.
- *Leaky Bucket (`LeakyBucketShaper`):* Реализует паттерн Traffic Shaping. Запросы, превышающие базовую пропускную способность, помещаются во внутреннюю очередь (`std::queue`). Выделенный фоновый поток (`queue_update_`) с заданной периодичностью извлекает запросы из очереди и сигнализирует об их готовности через механизм `std::promise`, разблокируя `std::future` на стороне клиента.

=== 2.3. Управление временем жизни (TTL) и фоновая очистка

Для предотвращения утечек памяти при работе с неограниченным потоком уникальных ключей реализован механизм Time-To-Live (TTL). Логика отслеживания времени жизни интегрирована в алгоритмы с помощью паттерна CRTP (шаблонный класс-обертка `StoredData`), который добавляет методы `IsExpired()` и `Prolong()` без модификации базового кода лимитеров.

Очистка устаревших ключей (`Eviction`) в `MutexStorage` работает в двух режимах:
- *Фоновая очистка (Background Cleaner):* При инициализации хранилища запускается отдельный поток (`cleaner_thread_`), который периодически блокирует хранилище и удаляет истекшие записи (`DoCleanup()`).
- *Ленивая очистка (Lazy Cleanup):* Активируется только при конкурентных обращениях к хранилищу (с проверкой через атомарные флаги `LazyState`), что снижает накладные расходы на синхронизацию при слабой нагрузке.

=== 2.4. Стратегии хранения ключей и потокобезопасность

Хранение состояния клиентов реализовано с учетом многопоточной конкуренции: \
*`MutexStorage`* использует `std::unordered_map`, где каждый клиент защищен собственным мьютексом. Обеспечивает высокий параллелизм, но требует выделения примитива синхронизации на каждый ключ. \
*`ShardedStorage`* группирует ключи в блоки (`DataNode`), защищая каждый шард одним мьютексом. 

Для оптимизации пропускной способности в шардированном хранилище применяется борьба с эффектом ложного разделения кэш-линий (False Sharing). Выделение памяти под шарды производится через кастомный `AlignedAllocator` (с использованием `_aligned_malloc` или `std::aligned_alloc`), а объекты располагаются в памяти с выравниванием по границе 64 байт (`kCacheLineSize`), что исключает аппаратные конфликты кэша процессора между рабочими потоками.
